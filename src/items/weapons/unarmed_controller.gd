...
func _physics_process(delta: float) -> void:
	...
	# Calculate idle guard targets
	var left_target = player.global_position + aim_dir * 12.0 + aim_dir.rotated(-PI/2) * 8.0
	var right_target = player.global_position + aim_dir * 12.0 + aim_dir.rotated(PI/2) * 8.0
	...
	if is_punching:
		...
		if next_punch_is_left:
			l_hand.accumulated_force += aim_dir * 15000.0
			r_hand.accumulated_force += (right_target - r_hand.pos) * 1500.0
		else:
			r_hand.accumulated_force += aim_dir * 15000.0
			l_hand.accumulated_force += (left_target - l_hand.pos) * 1500.0
